...
// IDF-LCDTest
// ===========
// Test of the ESP IDF LCD functions for the CYD
// See README.md for the gory details
//----------------------------------------------------------------------
...
void ClearScreen() {
  ...
  for (int i = 0; i < LCD_H_RES/BUF_DEPTH; i++) {
    esp_lcd_panel_draw_bitmap(ScrPanel, 0, i*BUF_DEPTH, LCD_H_RES, (i+1)*BUF_DEPTH, ScrBuf);
...
void setup() {
  Serial.begin(115200);
  delay(2000);
  Serial.println("IDF-LCDTest started");
...
#define BLK_LEN (24*32)
  uint16_t* red   = ScrBuf;
  uint16_t* green = ScrBuf + BLK_LEN;
  uint16_t* blue  = ScrBuf + 2*BLK_LEN;
  ...
  // All done
  Serial.println("IDF-LCDTest completed");
  vTaskSuspend(NULL);
}
